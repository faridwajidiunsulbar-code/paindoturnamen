    ...
    if (validActiveDivisions.length > 0) {
      ...
      validActiveDivisions.forEach(div => {
        ...
        uniqueDivisionsMap.set(dbId, {
          ...
          status: div.podiumOfficial ? 'completed' : (div.knockoutStage ? 'knockout_stage' : (div.groups.length > 0 ? 'group_stage' : 'pending')),
          podium_official: !!div.podiumOfficial,
          official_at: div.officialAt || div.officialPodium?.officialAt || null,
          revoked_at: div.revokedAt || null,
          podium_revoked_reason: div.podiumRevokedReason || null,
